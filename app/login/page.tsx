  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <p className="mt-3 text-muted-foreground font-medium">Sign in to access your dashboard</p>
      </div>
      ...
      <form onSubmit={handleLogin} className="space-y-6">
        ...
        <Button type="submit" size="lg" className="w-full h-12 font-bold uppercase tracking-widest rounded-xl" disabled={isLoading}>
          {isLoading ? (
            <Loader2 className="h-5 w-5 animate-spin" />
          ) : (
            <span className="flex items-center gap-2">
              Sign In <ShieldCheck className="h-4 w-4 opacity-70" />
            </span>
          )}
        </Button>
      </form>
      ...
      <div className="text-center text-sm font-medium pt-2">
        ...
        <a href="/signup" className="text-primary hover:underline font-bold">Sign Up</a>
      </div>
    </motion.div>
  )
}
...
export default function LoginPage() {
  return (
    <div className="flex min-h-screen items-center justify-center px-6 bg-background/50">
      <Suspense fallback={<div className="h-96 w-full max-w-md rounded-2xl bg-card/40 animate-pulse" />}>
        <LoginForm />
      </Suspense>
    </div>
  )
